...
export default class CognitoAuthenticator extends Base {
    ...
    _handleSignIn(user) {
        if (user.nextStep === this.cognito.nextStepOptions.DONE) {
    ...
    _handleNextStep(nextStep, params) {
        ...
        } else if (nextStep.signInStep === this.cognito.nextStepOptions.DONE) {
            return this._resolveAuth();
        } else if (nextStep === this.cognito.nextStepOptions.COMPLETE_AUTO_SIGN_IN) {
            return this.cognito.autoSignIn();
        } else if (nextStep.signInStep === this.cognito.nextStepOptions.CONFIRM_SIGN_IN_WITH_NEW_PASSWORD_REQUIRED) {
            return this._handleNewPasswordRequired(params);
        } else if (nextStep.signInStep === this.cognito.nextStepOptions.CONFIRM_SIGN_IN_WITH_SMS_CODE 
                    || nextStep.signInStep === this.cognito.nextStepOptions.CONFIRM_SIGN_IN_WITH_TOTP_CODE
                    || nextStep.signInStep === this.cognito.nextStepOptions.CONFIRM_SIGN_IN_WITH_CUSTOM_CHALLENGE) {
    ...
    async _submitChallengeResponse(answer) {
        ...
        if (authResult.nextStep === this.cognito.nextStepOptions.DONE) {
            return this._resolveAuth();
        }
